...
exports.getcareerpath = (req, res) => {
  ...
  try {
    db.query(
      "SELECT * FROM careerpath WHERE careercategoryid  =  $1 ORDER BY careercategoryid DESC",
      [careercategoryId],
      (err, results) => {
        if (err) {
          return res
            .status(400)
            .json({ error: "Could not retrieve careerpaths" });
        }
...
exports.getallcareerpath = (req, res) => {
  db.query("SELECT * FROM careerpath", (err, results) => {
    if (err) {
      return res.status(400).json({ error: "Could not retrieve careerpaths" });
    }
    return res.status(200).json(results.rows);
  });
};
